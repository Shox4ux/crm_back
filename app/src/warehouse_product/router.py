...
from app.src.warehouse_product.schema import WareProdRead, WareProdUpdate, WareProdWrite
...
async def create(
    ...
    old_wp = await dao.get_by_prod_id(prod_id=data.product_id)
    if old_wp:

        await dao.update(
            data=WareProdUpdate(quantity=old_wp.quantity + data.quantity),
            id=old_wp.id,
        )

    else:
        new_wp = await dao.create(data)
        if not new_wp:
            raise Exception()
    ...
    updated_product = await p_dao.update(
        product.id, ProductBase(active_quantity=product.active_quantity - data.quantity)
    )
...
async def update(
    ...
    wp_updated = await dao.update(data, id)
    if not wp_updated:
        raise Exception()
